...
for model in ["MSE", "SSIM", "NLPD", "MS_SSIM", "LPIPS", "DISTS"]:
  ...
  for subject in range(14):
    ...
    # 创建示例数据
    x = num1[18:36] # 替换为您的x列数据
    y = num2[18:36]  # 替换为您的y列数据
    c = []
    ...
    plt.ylabel(label+" predicted JND",fontsize=15)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)

    plt.savefig(model.lower()+"_fig/"+str(subject)+"_"+model.lower()+"_corr_blur.png")
    plt.show()
    # print(nums)
