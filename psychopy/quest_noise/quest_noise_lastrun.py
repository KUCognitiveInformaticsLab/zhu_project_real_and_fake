...
expName = 'quest_noise'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\京都大学\\Desktop\\zhu\\quest_noise\\quest_noise_lastrun.py',
...
for thisImageType in imageType:
    ...
    img3 = cv2.imread(source+imgType+".png",0)
    ...
    for level, condition in staircase:
        ...
        #print(roundLevel)
        image_1 = source+imgType+".png"
        image_2 = source+imgType+".png"
        ...
        # closed interval
        sigma = 0.001*(10**(3*level))
        np.random.seed(seed)
        noise = np.random.normal(0,sigma,imgTmp.shape)
        gauss_out = np.clip(imgTmp+noise, 0,1)
        gauss_out = np.uint8(gauss_out*255)
        cv2.imwrite(source+"tmp.png",gauss_out)
